...
# -----------------------------------------------------------
# Load artifact (everything comes from model file now)
# -----------------------------------------------------------
...
# per-patient bootstrap CIs (for patient display)
ci_A = artifact["ci_A"]
ci_B = artifact["ci_B"]
ci_AB = artifact["ci_AB"]


# -----------------------------------------------------------
# UI / Input construction helpers
# -----------------------------------------------------------
location_map = {
    0: "infratentorial",
    1: "supratentorial",
...
# Bootstrap patient-level CI
ciA_low  = float(ci_A["low"][0])
ciA_high = float(ci_A["high"][0])

ciB_low  = float(ci_B["low"][0])
ciB_high = float(ci_B["high"][0])

ciAB_low  = float(ci_AB["low"][0])
ciAB_high = float(ci_AB["high"][0])
...
with col1:
    ...
    st.write(f"CI (bootstrap): {ciA_low*100:.1f}% – {ciA_high*100:.1f}%")
    if binA:
        st.caption(f"Observed A (calibration): {binA['obs_rate']*100:.1f}% (n={binA['n']})")

with col2:
    st.subheader("Model B (trained on B)")
    st.write(f"**Risk:** {pB*100:.1f}%")
    st.write(f"CI (bootstrap): {ciB_low*100:.1f}% – {ciB_high*100:.1f}%")
    if binB:
        st.caption(f"Observed B (calibration): {binB['obs_rate']*100:.1f}% (n={binB['n']})")

with col3:
    st.subheader("Model AB (pooled A+B)")
    st.write(f"**Risk:** {pAB*100:.1f}%")
    st.write(f"CI (bootstrap): {ciAB_low*100:.1f}% – {ciAB_high*100:.1f}%")
    if binAB:
